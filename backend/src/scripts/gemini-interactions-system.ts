  ...
  private async fetch_and_emit_media(query: string, type: string) {
    try {
      if (type === "video") {
        ...
        // ALWAYS try Vertex AI Search first for every query.
        // Imagen generation is the ABSOLUTE LAST RESORT.
        const searchResult = await this.mediaSearch.searchImage(query);
        ...
        if (searchResult) {
          ...
          console.log(`Sent search image to client: ${searchResult.title}`);
          return {
            success: true,
            mediaType: "image",
            title: searchResult.title,
        ...
        try {
          ...
          console.log(`Sent generated image to client: ${query}`);
          return {
            success: true,
            mediaType: "image",
            title: query,
          ...
          console.error("Imagen generation also failed:", genError);
        }
      }
      return { success: false, error: "No media found" };
    } catch (error) {
      console.error("fetch_and_emit_media failed:", error);
...
Does this message indicate at-risk behavior that requires adult intervention? Consider both the explicit content and the emotional context.`;
  ...
  /**
   * Generate a photorealistic image using Imagen 3 for show_visual.
   * Uses a different prompt than sprite generation (no white background, no sticker style).
   */
  private async generateVisualImage(query: string): Promise<string> {
    const cacheKey = `visual_${query.toLowerCase()}`;
    const cached = this.generatedSpriteCache.get(cacheKey);
    if (cached) {
      return cached;
    }

    console.log(`Imagen: Generating visual image for "${query}"...`);
    const response = await this.AI.models.generateImages({
      model: "imagen-3.0-generate-001",
      prompt: buildVisualImagePrompt(query),
      config: {
        numberOfImages: 1,
      },
    });
    ...
    const imageBytes = response.generatedImages?.[0]?.image?.imageBytes;
    if (!imageBytes) {
      throw new Error(`No generated image returned for "${query}"`);
    }
