...
export class MediaSearchService {
  ...
  async searchImage(query: string): Promise<ImageResult | null> {
    if (!this.cseApiKey || !this.cseId) {
      console.warn(
        "MediaSearchService: Missing GOOGLE_CSE_API_KEY or GOOGLE_CSE_ID",
      );
      return null;
    }
    ...
    try {
      ...
      if (data.items && data.items.length > 0) {
        ...
        console.warn(
          "MediaSearchService: Custom Search API returned no items. Response:",
  ...
  async searchVideo(query: string): Promise<VideoResult | null> {
    ...
    try {
      ...
      if (data.items && data.items.length > 0) {
        ...
        console.warn(
          "MediaSearchService: YouTube API returned no items. Response:",
          JSON.stringify(data, null, 2),
        );
      }
      return null;
